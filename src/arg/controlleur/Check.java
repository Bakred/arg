...
public class Check {
	...
	public static void CheckHP(Player joueur, Mob monstre) throws IOException, InterruptedException {
		if (joueur.getHP() <= 0) {
			Message.Msg("Vous avez été vaincu !");
			Pause.PauseAff(800);
			Message.Confirm();
	...
	public static int CheckWeakResist(Player joueur, Mob monstre, int num, String nomMagie, String type)
		...
		if (joueur.isYouTurn()) {
			if (type.equals("Magie")) {
				if (monstre.getWeak().equals(nomMagie) || monstre.getWeak().equals(type)) {
					num = num * 2;
					Message.Msg("C'est trés éfficace ! ");
				}
				if (monstre.getResist().equals(nomMagie) || monstre.getResist().equals(type)) {
					double tamp;
		...
		if (joueur.isYouTurn()) {
			if (type.equals("Attaque")) {
				if (monstre.getWeak().equals(type)) {
					num = num * 2;
					Message.Msg("C'est trés éfficace ! ");
				}
				if (monstre.getResist().equals(type)) {
					double tamp;
					tamp = ((double) num / ((double) 2));
					num = ((int) tamp);
					Message.Msg("C'est peu éfficace... ");
